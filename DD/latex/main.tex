\documentclass[11pt,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{import}
\usepackage{listings}
\usepackage[english]{babel}
\usepackage{array}
\usepackage[export]{adjustbox}
\usepackage{minted}
\usepackage{float}
\usepackage{multirow}
\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=black
}
\begin{document}

\begin{figure}[t]
    \includegraphics[scale=0.8]{Images/logo_polimi.png}
    \centering
\end{figure}

\begin{titlepage}
    \newcommand{\HRule}{\rule{\linewidth}{0.5mm}}
    \center
    \textsc{\LARGE POLITECNICO DI MILANO}\\[1.5cm]
	
	\textsc{\Large SOFTWARE ENGINEERING II}\\[0.5cm]
	
	\HRule\\[0.4cm]
	    {\huge \textsc{SafeStreets - \\Design Document}}\\[0.4cm]
	    Version 1.0
    \HRule\\[1.5cm]
	
	\begin{minipage}{1\textwidth}
		\begin{flushleft}
			\large
			\textit{Authors}\\
			Iacopo \textsc{Marri}\\
			Manuel \textsc{Salamino}\\
			Steven Alexander \textsc{Salazar Molina}
		\end{flushleft}
	\end{minipage}
	
	\vfill\vfill\vfill % Position the date 3/4 down the remaining page
	
	{\large\today}
	
\end{titlepage}


\newpage

\tableofcontents

\include{Files/1_introduction}
\include{Files/2_architectural_design}
\include{Files/3_user_interface_design}
\include{Files/4_requirements_traceability}
\include{Files/5_implementation_integration_test}
\include{Files/6_effort_spent}
\include{Files/7_references}
% ho tolto References perché ho notato che Diego/loris, lo scarlatti, giò e negri non avevano questo sezione. ho letto il loro assigment ed era come il nostro (indicato References come sezione 6) ma non l'hanno fatto quindi boh, lo chiederei al prof nel caso perché c'è anche una sottosezione nella sezione 1 in cui chiede 'document references'
%\include{Files/references}

\end{document}
